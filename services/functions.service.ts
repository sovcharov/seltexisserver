...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  getPriceListQuery () {
    ...
    //query = `SELECT i.id, i.description, i.comment, i.price, i.stock, i.ordered, i.msk, n.number, m.fullName as manufacturerFullName, n.main, img.id as imgId FROM seltexru.inventory as i, seltexru.inventoryNumbers as n, seltexru.inventoryManufacturers as m, seltexru.inventoryImages as img  where i.id = n.inventoryId and n.manufacturerId = m.id and i.id = img.inventoryId and (${midQuery}) and (i.description not like '%core%' and i.comment not like '%core%')`
    return(query);
  }
}
